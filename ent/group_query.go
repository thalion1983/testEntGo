...
// QueryPeoples chains the current query on the "peoples" edge.
func (gq *GroupQuery) QueryPeoples() *PeopleQuery {
	query := (&PeopleClient{config: gq.config}).Query()
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withPeoples: gq.withPeoples.Clone(),
...
// WithPeoples tells the query-builder to eager-load the nodes that are connected to
// the "peoples" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithPeoples(opts ...func(*PeopleQuery)) *GroupQuery {
	query := (&PeopleClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withPeoples = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withPeoples; query != nil {
		if err := gq.loadPeoples(ctx, query, nodes,
			func(n *Group) { n.Edges.Peoples = []*People{} },
			func(n *Group, e *People) { n.Edges.Peoples = append(n.Edges.Peoples, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadPeoples(ctx context.Context, query *PeopleQuery, nodes []*Group, init func(*Group), assign func(*Group, *People)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.PeoplesTable)
		s.Join(joinT).On(s.C(people.FieldID), joinT.C(group.PeoplesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(group.PeoplesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.PeoplesPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*People](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "peoples" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
